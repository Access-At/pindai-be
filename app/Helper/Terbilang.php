...
class Terbilang
{
    ...
    public function parse($num = '', $sep = '')
    {
        ...
        if ( ! in_array($sep, $this->separators, true)) {
            throw new Exception('Harap gunakan koma atau titik sebagai pemisah');
        }

        $result = [];
        $num = trim((string) $num, ' ,.');
        if (str_starts_with($num, '-') && trim($num, ',-.0') !== '') {
            $result[] = 'minus';
        }

        if (($sep_pos = mb_strrpos($num, $sep))) {
            $result[] = $this->getResult($this->read(mb_substr($num, 0, $sep_pos))->result);
            $result[] = 'koma';
            $result[] = $this->spell(mb_substr($num, $sep_pos))->getResult();
        } else {
            $sep_alt = $this->separators[array_search($sep, $this->separators, true) ^ 1];
            $sep_alt_pos = mb_strpos($num, $sep_alt);

            if ($sep_alt_pos && str_starts_with($num, '0') || mb_substr_count($num, $sep_alt) === 1 && mb_strlen(mb_substr($num, $sep_alt_pos)) !== 4) {
                $result[] = $this->getResult($this->read(mb_substr($num, 0, $sep_alt_pos))->result);
                $result[] = 'koma';
                $result[] = $this->spell(mb_substr($num, $sep_alt_pos))->getResult();
            } else {
    ...
    protected function read($num = '')
    {
        ...
        if (mb_strlen($num) > 108) {
            throw new Exception('Maaf, angka yang anda masukkan terlalu besar');
        }

        $this->result = [];

        while (($len = mb_strlen($num)) > 0) {
            $s_index = (int) floor(($len - 1) / 12);
            $num_ = mb_substr($num, 0, $len - $s_index * 12);
            while (($len_ = mb_strlen($num_)) > 0) {
                $s_index_ = (int) floor(($len_ - 1) / 3);
                $num__ = mb_substr($num_, 0, $len_ - $s_index_ * 3);

                while (($len__ = mb_strlen($num__)) > 0) {
                    $s_index__ = (int) floor(($len__ - 1) / 2);
                    $num___ = mb_substr($num__, 0, $len__ - $s_index__ * 2);
                    ...
                    $num__ = ltrim(mb_substr($num__, $len__ - $s_index__ * 2), '0');
                }

                $this->result[] = $this->suffixes[3][$s_index_]; // ribu, juta, miliar
                $num_ = ltrim(mb_substr($num_, $len_ - $s_index_ * 3), '0');
            }

            $this->result[] = $this->suffixes[4][$s_index] . ','; // triliun, septiliun, ..., untrigintiliun
            $num = ltrim(mb_substr($num, $len - $s_index * 12), '0');
        }
